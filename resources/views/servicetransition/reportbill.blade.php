...
?>
 ...
 @foreach ( $order->servicetransition as $t )
  <tr>
   <td>{{$order->id}}-{{$m}}</td>
    <td> {{$t->servicelistinhospital->servicename}}</td>
   <td> <?php  echo round( $t->unit , 1);  ?>  </td>
	<?php if (($i ==0) || ($i ==2)) { ?>
   <td>  <?php  echo round($t->servicelistinhospital->price , 2);  ?>  </td>
   <?php 

$price_before_vat_and_discount = $t->servicelistinhospital->price * $t->unit ;
   ?>
    ...
    <td>      <?php  echo round($price_before_vat_and_discount, 2);  ?> </td>
	 ...
	 $tadjust = $tadjust + $t->receiveable_amount;
...
<div style="width:20%;float:left;" >
<b> Paid:</b> <?php  echo round($order->paid, 2);  ?>TK.   
    </div>	
	
	
<div style="width:35%;float:left;" >
<b> Adjust Adv. payemnt:</b> <?php  echo round($order->due_adjust_from_advance, 2);  ?>TK.   
    </div>		
...
{ ?>
    ...
    <div style="height:10px;" id="one" >
    <div style="width:50%; float:left;" >
<b>Doctor Name:</b> {{$order->doctor->name}} <br>
	</div>
    <div style="width:50%;float:left;" >
 <b>Commission:</b> {{$order->commission}} TK
    </div>
