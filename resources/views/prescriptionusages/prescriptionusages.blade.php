...
$(document).ready(function(){
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('prescriptionusages.index') }}",
        columns: [
...
$('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('prescriptionusages.store') }}",
    method:"POST",
  ...
  {
   $.ajax({
    url:"{{ route('prescriptionusages.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
 ...
 $(document).on('click', '.edit', function(){
  ...
  $('#form_result').html('');
  $.ajax({
   url:"/prescriptionusages/"+id+"/edit",
   dataType:"json",
   success:function(html){
    $('#name').val(html.data.usage);
 ...
 $('#ok_button').click(function(){
  $.ajax({
   url:"prescriptionusages/destroy/"+user_id,
   beforeSend:function(){
