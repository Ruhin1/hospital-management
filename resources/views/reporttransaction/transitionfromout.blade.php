...
<h2> Pathology Test which was done from Other Diagonistic Center </h2>

<input type="hidden" id="percentageofagent" value="0" name="percentageofagent" >	  

<div  class="container" style="background-color:#EEE8AA; "  >
  <span id="form_result"></span>
         <form method="post" action="{{ route('pathologytestfromother.store') }}"   id="sample_form" class="form-horizontal" enctype="multipart/form-data">
          @csrf
  ...
  <div class="row">
   ...
   <div class="col-6">
    Source:
	 <select id="supplier"  class="form-control "  name="supplier" style="width:300px;" required>
    </select>
    </div>
...
//////////////////////////// Show record 
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('pathologytestfromother.index') }}",
        columns: [
...
function fetch()
{
	 ...
	 $("#customer_id").select2();
	  $.ajax({
   url:"pathologytestfromother/mlist",
   dataType:"json",
	   ...
	   ////////////////////fetch data for Customer dropdown menu ////////////////////////////
		...
		$("#agent").html("");
		 ...
		 $("#supplier").html("");  
	  	  	  		...
	  	  	  		var supplier = "<option  value='' ></option>"; 
                    ...
                    if (len>0) {
                        for (var i = 0; i<len; i++) {
                             ...
                             var doctor = "<option     data-name='"+name+"'   value='"+id+"'>"+name+"</option>"; 
							

                             $("#supplier").append(doctor);
                        }
                    }
...
$('#sample_form').on('submit', function(event){
  ...
  console.log('B');
   $.ajax({
    url:"{{ route('pathologytestfromother.store') }}",
    method:"POST",
...
$('#formModal').modal('hide');
 ...
 $('#ok_button').click(function(){
  ...
  $.ajax({
   url:"pathologytestfromother/destroy/"+user_id,
   beforeSend:function(){
