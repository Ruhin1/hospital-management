...
<style>
        ...
        u {
            ...
            display: inline-block; /* This ensures that the margin affects only the bottom */
        }
...
$total_due_discount_patho=0;
$total_due_amount_collec_patho=0;
$total_pathology_test=0;


$total_unit_service = 0;
$total_discount_service = 0;
$total_receiveable_service = 0;
$total_service_sales=0;
$total_due_amount_servie = 0  ;
$total_due_discount = 0;
$total_admission_due_collection = 0;
$total_admission_due_discount =0;

 $total_due_collection_medicine= 0;
 $total_due_discount_medicine = 0;
 $total_item_sales_medicine =0;
 $total_due_amount_surgery= 0;
$total_due_discount_surgery=0;
$total_number_surgery=0;
  ?>
...
<div  class="container">
  ...
  <hr>
      ...
      <div id="three" >
	   ...
	   <h2> Today's Sales-Expenses Statemnet: <?php echo date("d/m/y") ;  ?> </h2>
...
Tests are done from Other Diagonistic on due: {{$due_by_cash_to_other}} TK
  ...
  <?php if (!$withdrawl_from_investors->isEmpty())  { ?>
    <h2>   Money Withdrwal  </h2>
      <hr>
      <table class="table">
      <thead>
        <tr>
      ...
      @foreach($withdrawl_from_investors as $withdral)
      <?php  
      $total_withdrawl = $total_withdrawl + $withdral->amount_invest;
      
      ?>
      
      <tr>
          <th >{{$withdral->sharepartner->name}}</th>
          
      <td> <?php echo  number_format($withdral->amount_invest, 2, '.', ',');?>	 </td>      
        </tr>
    @endforeach
      </tbody>
    </table>
    <hr>
    <span > So Total Amoutnt of Money Withdrawl:  </span> <?php echo $total_withdrawl  ?> TK. 
...
<span > So Total amount Refund: </span> <?php echo $patientferot= $money_back_customerrelease_time+$cabine_refund+ $pathology_refund + $medicine_refund + $surgery_refund + $doctor_refund + $service_refund + $admission_refund ?> TK 
 ...
 <span > Doctor's Share/surgical Fees:   </span> <?php echo $totaldoctorshare ?> TK  <br> 
     <span > Money withdral :   </span> <?php echo $total_withdrawl ?> TK <br> 
  
  (+) <br >
  <hr style="width:50%">
<span > Total  Expenses  </span> <?php echo $khoroch= $totalcommission+$totalexcost+ $totalmedicinemotkroy + $totaldoctorshare + $total_receiv_out +$totalbeton + $totalkhoroch + $totaldoctorcommission + $reagent_total_purchase->total_price_amount + $total_withdrawl ?> TK 
 <br>
 <br>
...
if yes then exexute  -->
 ...
 <br><br>
<?php  $i=0;	$j=0;  if ( (!$income_from_pathology_test->isEmpty()) 
	or (!$income_from_due_payment_pathology->isEmpty()) )  {  ?>
<h2> <u> Pathology </u>  </h2>
 ...
 <th scope="col">  Receiveable Amnt(TK)= Total - Discount    </th>
    </tr>
  </thead>
  <tbody>
...
$total_pathology_test = $total_pathology_test + $inc->total_unit;
...
<br>
<span >   Total Sale:          </span> <?php echo number_format($net_income_from_pth,2,'.',',');  ?> TK <br>

<span > Total Discount:      </span> <?php echo number_format($total_discoubt_in_patho,2,'.',',');  ?> TK <br>
(-) <br>
<hr style="height:2px; 	width:30%; text-align:left; " >
<br>
<b><span >  Receiveable Amnt.:  </span> </b>
...
<hr style="height:2px; 	width:30%; text-align:left; " >
   <b> <span >   Sales in Cash:     </span> <?php $mot_nogod_income_from_pathology =  $totalincome_from_pathology - $total_due_patho - $total_adjust_patho ?> <?php echo number_format($mot_nogod_income_from_pathology,2,'.',',');  ?>TK <br></b>
  <br> 
   ...
   <?php if (!$income_from_due_payment_pathology->isEmpty())  { ?>
    ...
    $total_due_amount_collec_patho= $total_due_amount_collec_patho + $due_paid->totalamount;
     $total_due_discount_patho = $total_due_discount_patho + $due_paid->duediscount;
    ...
    <br>
    <span > Total Due Collection :   </span> <?php echo $total_due_amount_collec_patho  ?> TK<br>
     <span > Discount  :   </span> <?php echo $total_due_discount_patho  ?> TK<br>   
    (-)
	<hr style="height:2px; 	width:30%; text-align:left; " >
        <span ><b> Due Paid  :</b>   </span> <?php echo $total_due_paid_pathology  ?> TK<br> 
...
<br>
   ...
   <span > Total Invoice:   </span> {{$pathology_voucher_number }}<br>
   <span > Total Test:   </span> {{$total_pathology_test }}<br>
   <span > Total Sales in Cash:   </span> <?php echo round($mot_nogod_income_from_pathology,2)  ?> Tk.<br>
    <span > Total Due Paid :   </span><?php echo round($total_due_paid_pathology,2)  ?> Tk.<br>  
   (+)
  <hr style="height:2px; 	width:30%; text-align:left; " >
  

	     <?php $mot_nogod_income_from_pathology= $mot_nogod_income_from_pathology + $total_due_paid_pathology; ?>
  
  
<b> Total Collection in Cash:  </b> <?php echo round($mot_nogod_income_from_pathology,2)  ?> Tk.
 ...
 <br><br>
<?php  $i=0;	$j=0;  
if ((!$incomefromservice->isEmpty()) or (!$income_from_due_payment_service->isEmpty()))  {  ?>
<h2 >  <u> Service </u>   </h2>
  
 <br> 
  
    <?php if (!$incomefromservice->isEmpty())  { ?> 
  <h3>Sales</h3> 
...
$receiveable_amnt_servie= $inc->charge - $inc->discount;

$total_unit_service = $total_unit_service + $inc->unit;
$total_discount_service = $total_discount_service + $inc->discount;
$total_receiveable_service = $total_receiveable_service + $receiveable_amnt_servie;
$total_service_sales = $total_service_sales + $inc->charge;
...
<hr>
Total Sales : {{$total_service_sales}} TK <br>
Discount  : {{$total_discount_service}} TK<br>
(-)

<hr style="height:2px; 	width:30%; text-align:left; " >
<b>Receiveable Amnt. :</b> {{ $total_receiveable_service }} Tk.<br>
(-) Due: {{ $bakiincomefromservice }} Tk.<br>
(-) Paid  by Adv. Deposit: {{$adjustment_service}} TK<br>
<hr style="height:2px; 	width:30%; text-align:left; " >
<b>Sales in Cash:</b> {{$nogodincomefromservice}}TK
...
$total_due_amount_servie = $total_due_amount_servie + $due_paid->totalamount  ;
$total_due_discount = $total_due_discount + $due_paid->duediscount;
$j++;
...
@endforeach
    ...
    <br>
    <span > Total Due Collection :   </span> <?php echo $total_due_amount_servie  ?> TK<br>
     <span > Discount  :   </span> <?php echo $total_due_discount  ?> TK<br>   
    (-)
	<hr style="height:2px; 	width:30%; text-align:left; " >
        <span ><b> Due Paid  :</b>   </span> <?php echo $total_due_paid_service  ?> TK<br> 
  ...
  <?php } ?> 
   ...
   <span > Total Sales in Cash:   </span> <?php echo round($nogodincomefromservice,2)  ?> Tk.<br>
    <span > Total Due Paid :   </span><?php echo round($total_due_paid_service,2)  ?> Tk.<br>  
   (+)
  <hr style="height:2px; 	width:30%; text-align:left; " >
  

	     <?php $nogodincomefromservice= $nogodincomefromservice + $total_due_paid_service; ?>
  
  
<b> Total Collection in Cash:  </b> <?php echo round($nogodincomefromservice,2)  ?> Tk.
...
if yes then exexute  -->
 ...
 <br><br>
<?php $sumadmissionfee=0;  $i=0;$j=0; if ((!$admissionfee->isEmpty()) 
	or (!$income_from_due_payment_admissionfee->isEmpty()) )  { ?>

<h2> <u> Admission </u> </h2>
     <?php if (!$admissionfee->isEmpty())  { ?>
<h3 >  Sales  </h3>
  ...
  <thead>
    ...
    <th>No.</th>
	  ...
	  <th scope="col">   Total (TK.) </th>
 <th scope="col">   Discount (TK.) </th>
 <th scope="col">   Receiv. Amnt (TK.) </th>
...
(+)
<hr>
Total Sales : {{$sum_grossamnt_amission_fee}} TK <br>
Discount  : {{$discount_grossamnt_amission_fee}} TK<br>
(-)

<hr style="height:2px; 	width:30%; text-align:left; " >
<b>Receiveable Amnt. :</b> {{ $otal_recivable_admission_fee }} Tk.<br>
(-) Due: {{ $due_admission_fee }} Tk.<br>

<hr style="height:2px; 	width:30%; text-align:left; " >
<b>Sales in Cash:</b> {{$paid_admission_fee}}TK
...
$total_admission_due_collection = $total_admission_due_collection + $due_paid->totalamount;
$total_admission_due_discount = $total_admission_due_discount + $due_paid->duediscount;
...
@endforeach
    ...
    <br>
    <span > Total Due Collection :   </span> <?php echo round($total_admission_due_collection,2)  ?> TK<br>
     <span > Discount  :   </span> <?php echo round($total_admission_due_discount,2)  ?> TK<br>   
    (-)
	<hr style="height:2px; 	width:30%; text-align:left; " >
        <span ><b> Due Paid  :</b>   </span> <?php echo round($total_due_paid_admission_fee,2)  ?> TK<br> 
 ...
 <br><br>
  ...
  <?php } ?> 
   ...
   <span > Total Sales in Cash:   </span> <?php echo round($paid_admission_fee,2)  ?> Tk.<br>
    <span > Total Due Paid :   </span><?php echo round($total_due_paid_admission_fee,2)  ?> Tk.<br>  
   (+)
  <hr style="height:2px; 	width:30%; text-align:left; " >
  

	     <?php $paid_admission_fee= $paid_admission_fee + $total_due_paid_admission_fee; ?>
  
  
<b> Total Collection in Cash:  </b> <?php echo round($paid_admission_fee,2)  ?> Tk.
...
if yes then exexute  -->
 ...
 <br><br>
<?php
//dd($cabinefeetransition);
 $sumcabinefee=0;  $i=0;  if (!$cabinefeetransition->isEmpty())  { ?>
<h2 > <u>  Cabine Fees</u>   </h2>
...
if yes then exexute  -->
 ...
 <br><br>
<?php   $i=0; $j=0; if ((!$medicinetransition->isEmpty()) or (!$income_from_due_payment_medicine->isEmpty()))  { ?>
...
$total_item_sales_medicine = $total_item_sales_medicine + $med->quantity;
...
<br>
<span >   Total Sale:          </span> <?php echo number_format($net_income_from_medicine_from_all_itteration_of_loop,2,'.',',');  ?> TK <br>

<span > Total Discount:      </span> <?php echo number_format($total_discount_in_medicine,2,'.',',');  ?> TK <br>
(-) <br>
<hr style="height:2px; 	width:30%; text-align:left; " >
<br>
<b><span >  Receiveable Amnt.:  </span> </b>
    <?php echo number_format($totalincome_from_medicine,2,'.',',');  ?> TK <br>
    <span > (-)  Sale on Due:  </span>  {{$total_due_medicine}} TK <br>
  
    (-) <span >  Paid by Advance Cash :  </span>  {{$medicine_adv}} TK <br>
<hr style="height:2px; 	width:30%; text-align:left; " >

   <b> <span >   Sales in Cash:     </span> 
<?php $mot_nogod_sell_from_medicine =  $totalincome_from_medicine - $total_due_medicine - $medicine_adv ?>
    <?php echo number_format($mot_nogod_sell_from_medicine,2,'.',',');  ?>TK <br></b>
  <br> 
 ...
 $total_due_collection_medicine=   $total_due_collection_medicine + $due_paid->totalamount;
 $total_due_discount_medicine = $total_due_discount_medicine + $due_paid->duediscount;
 ...
 <hr>
     ...
     <br>
    <span > Total Due Collection :   </span> <?php echo round($total_due_collection_medicine,2)  ?> TK<br>
     <span > Discount  :   </span> <?php echo round($total_due_discount_medicine,2)  ?> TK<br>   
    (-)
	<hr style="height:2px; 	width:30%; text-align:left; " >
        <span ><b> Due Paid  :</b>   </span> <?php echo round($total_due_paid_medicine,2)  ?> TK<br> 
   ...
   <span > Total Invoice:   </span> {{$medicine_voucher_number }}<br>
   <span > Total Item Sales:   </span> {{$total_item_sales_medicine }}<br>
   <span > Total Sales in Cash:   </span> <?php echo round($mot_nogod_sell_from_medicine,2)  ?> Tk.<br>
    <span > Total Due Paid :   </span><?php echo round($total_due_paid_medicine,2)  ?> Tk.<br>  
   (+)
  <hr style="height:2px; 	width:30%; text-align:left; " >
  

	     <?php $mot_nogod_sell_from_medicine= $mot_nogod_sell_from_medicine + $total_due_paid_medicine; ?>
  
  
<b> Total Collection in Cash:  </b> <?php echo round($mot_nogod_sell_from_medicine,2)  ?> Tk.
...
if yes then exexute  -->
 ...
 <br><br>
<?php  $i=0; $j=0; if ((!$surgerytransaction->isEmpty()) or (!$income_from_due_payment_surgery->isEmpty()) ) { ?>

<h2> <u style="margin-bottom:10px"> Surgery/OT Departments: </u> </h2>
<br>
...
<h3 >Sales  </h3>
 ...
 <th scope="col"> Receiveable Amnt(TK.)</th>
    </tr>
  </thead>
  <tbody>
...
$net_sell_from_surgery_from_all_itteration_of_loop = $net_sell_from_surgery_from_all_itteration_of_loop + $net_sell_in_surgery_in_every_single_iiteration ;
		$total_number_surgery= $total_number_surgery + $sur->total;
	$i++;
...
<br>
<span >   Total Sale:          </span> <?php echo number_format($net_sell_from_surgery_from_all_itteration_of_loop,2,'.',',');  ?> TK <br>

<span > Total Discount:      </span> <?php echo number_format($total_discount_in_surgery,2,'.',',');  ?> TK <br>
(-) <br>
<hr style="height:2px; 	width:30%; text-align:left; " >
<br>
<b><span >  Receiveable Amnt.:  </span> </b>
    <?php echo number_format($totalsell_from_surgery_by_substracting_discount,2,'.',',');  ?> TK <br>
    <span > (-)  Sale on Due:  </span>  {{$total_due_surgery}} TK <br>
  
    (-) <span >  Paid by Advance Cash :  </span>  {{$total_adjust_surgery}} TK <br>
<hr style="height:2px; 	width:30%; text-align:left; " >

   <b> <span >   Sales in Cash:     </span> 

 <?php $mot_nogod_sell_from_surgery =  $totalsell_from_surgery_by_substracting_discount - $total_due_surgery - $total_adjust_surgery ?> 



    <?php echo number_format($mot_nogod_sell_from_surgery,2,'.',',');  ?>TK <br></b>
  <br> 
...
$total_due_amount_surgery= 	$total_due_amount_surgery + $due_paid->totalamount;
$total_due_discount_surgery= 	$total_due_discount_surgery + $due_paid->duediscount;
...
<hr>
     ...
     <br>
    <span > Total Due Collection :   </span> <?php echo round($total_due_amount_surgery,2)  ?> TK<br>
     <span > Discount  :   </span> <?php echo round($total_due_discount_surgery,2)  ?> TK<br>   
    (-)
	<hr style="height:2px; 	width:30%; text-align:left; " >
        <span ><b> Due Paid  :</b>   </span> <?php echo round($total_due_paid_surgery,2)  ?> TK<br> 
...
<br>
  ...
  <?php } ?> 
   ...
   <span > Total Invoice:   </span> {{$surgery_voucher_number }}<br>
   <span > Total Operation:   </span> {{$total_number_surgery }}<br>
   <span > Total Sales in Cash:   </span> <?php echo round($mot_nogod_sell_from_surgery,2)  ?> Tk.<br>
    <span > Total Due Paid :   </span><?php echo round($total_due_paid_surgery,2)  ?> Tk.<br>  
   (+)
  <hr style="height:2px; 	width:30%; text-align:left; " >
  

	     <?php $mot_nogod_sell_from_surgery= $mot_nogod_sell_from_surgery + $total_due_paid_surgery; ?>
  
  
<b> Total Collection in Cash:  </b> <?php echo round($mot_nogod_sell_from_surgery,2)  ?> Tk.
...
<!----------------------------------------------------------------------------------------------->
 ...
 <br><br>
<?php if ((!$doctortransition->isEmpty()) or  (!$income_from_due_payment_doctor->isEmpty())   )  { ?>
<h2 > <U> Outdoor Doctor/ Doctor Visit :</u>  </h2>
 ...
 <th scope="col"> Paid by Advance Payment   </th>
...
<?php } ?> 
   ...
   <?php if (!$income_from_due_payment_doctor->isEmpty())  { ?>
    ...
    <b> So Total Sale From Outdoor Doctor/ Doctor visit</b>   = Total Due Collection + Total Sale in Cash = <?php echo $total_due_paid_dcotor  ?> + <?php echo $total_paid_by_cash_doctor  ?> = <?php echo ($total_due_paid_dcotor + $total_paid_by_cash_doctor )?> TK 
       
<?php  $total_paid_by_cash_doctor =    $total_due_paid_dcotor + $total_paid_by_cash_doctor; ?>
...
if yes then exexute  -->
  ...
  <?php if (!$income_from_investors->isEmpty())  { ?>
    <h2>   Investment  </h2>
      <hr>
      <table class="table">
      <thead>
      ...
      @foreach($income_from_investors as $invest)
      <?php  
      $total_investment = $total_investment + $invest->amount_invest;
      
      ?>
      
      <tr>
          <th >{{$invest->sharepartner->name}}</th>
          
      <td> <?php echo  number_format($invest->amount_invest, 2, '.', ',');?>	 </td>      
        </tr>
    @endforeach
      </tbody>
    </table>
    <hr>
    <span > So Total Amoutnt of Salary Given:  </span> <?php echo $total_investment  ?> TK. 
...
$total_due_paid_service;
 ?>
  ...
  <h3> <u> Income Summary </u> </h3>
  <br>
  <br>
<b  >  Income in Cash </b> <br><br> 
  ...
  <span > Collection from Long-term treatment's installment Fees:      </span> 
  <?php echo number_format($long_term_service_income,2,'.',',') ?> TK <br> 
  ...
  <span >  Pathology:   </span> <?php echo number_format($mot_nogod_income_from_pathology,2,'.',','); ?> TK <br>
   ...
   <span > From Outdoor Doctors:   </span> <?php echo $total_paid_by_cash_doctor ?> TK <br>
      <span > From Service Charge:   </span> <?php echo $nogodincomefromservice ?> TK <br>
       <span > Investment:   </span> <?php echo $total_investment ?> TK <br>
 
 
 
 
(+) <hr >
  <span > SO Total Income in Cash:  </span> <?php $final_income= $long_term_service_income + $total_reagent_back +  $total_paid_by_cash_doctor+ $sumcabinefee + $advance_amount + $paid_admission_fee + $nogodincomefromservice + $mot_nogod_sell_from_surgery + $mot_nogod_sell_from_medicine  +$mot_nogod_income_from_pathology + $totalmedicine_cash_back_com +$total_investment ?> <?php echo round($final_income,2); ?>   TK <br>
...
<h2 > <u> Summary: 	    <?php echo date("d/m/y") ;  ?> </u>
</h2>
<div  style="width:50%; float:left;">
<hr>
  <span > <b> Income Summary </b>
  <br>
  <br>
<b  >  Income in Cash </b> <br><br> 
...
<span > So Total amount Refund: </span> <?php echo $patientferot= $money_back_customerrelease_time+$cabine_refund+ $pathology_refund + $medicine_refund + $surgery_refund + $doctor_refund + $service_refund + $admission_refund ?> TK 
 ...
 <span > Doctor's Share/surgical Fees:   </span> <?php echo $totaldoctorshare ?> TK  <br> 
  ...
  (+) <br >
  <hr style="width:50%">
<span > So Total amount of Expenses for Buying Products/Services </span> <?php echo $khoroch= $totalcommission+$totalexcost+$reagent_total_purchase->total_price_amount + $totalmedicinemotkroy + $totaldoctorshare + $total_receiv_out +$totalbeton + $totalkhoroch + $totaldoctorcommission + $total_withdrawl ?> TK 
 <br>
 <br>
 <h6 >  <b> Products/ Services that purchased On Credit:</b> </h6>
