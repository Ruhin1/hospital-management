...
<h6 style="color:red;">আপনি যে দুই তারিখসহ ও দুই তারিখের মধ্যবর্তী সকল ডেটের -  দেয়া কমিশন দেখতে চান সেই তারিখ সিলেক্ট করেন। 
...
<form      method="post"  action="{{route('agenttransaction.fetchreport')}}"   >
 @csrf
...
<P>
  Agent Name:
  <select  style="width:300px;" name="agent" id="agent" required>
    <option value="99999999999999">ALL</option>
  @foreach ( $agentdetail as $p  )
...
<select  style="width:300px;" name="doctor" id="doctor" required>
  <option value="88888888888888">ALL</option>
@foreach ( $doctor as $p  )
  <option value="{{$p->id}}">{{$p->name}}</option>
  @endforeach
</select>
