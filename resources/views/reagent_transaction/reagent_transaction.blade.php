...
<body id="bodysellcorner">
    ...
    @endif
		...
		<h2> Buy/ Buy Return Reagents</h2>
  <span id="form_result"></span>
	
		<form method="post" action="{{ route('reagentransaction.store') }}"   id="sample_form" class="form-horizontal" enctype="multipart/form-data">
          @csrf
		   ...
		   <div class="col-6"> 
		Supplier Name  :	<select id="medicinecomapnyname_id"  class="form-control medicinecomapnyname_id" placeholder="Select One" name="supplier"  style="width:100%;" required   >
   </select> 
             </div>
		 <div class="col-2"> 	 
		<input type="radio" id="purchase" name="transitiontype" value="1" required >
<label for="purchase"> Buy </label><br>
...
<div class="col-4"> 
  ...
  <p>	 
			 ...
			 <table class="table" id="products_table">
                ...
                <tbody class="addmoreproduct">
                    <tr id="product0">
                        <td>
       <select id="medicine_name"  class="form-control medicine_name"  name="reagent_name[]"  required   style='width: 200px;'>  
  
		</select>
                        </td>
                        <td>
						  <input type="text" autocomplete="off"  name="unit_price[]" id="unit_price" class="form-control numbers  unit_price" required style="width:300px;"  />
                           
                        </td>
						<td>
						  <input type="text" autocomplete="off" name="quantity[]" id="quantity" class="form-control numbers quantity" required style="width:300px;" />
                        </td>	
						<td>
 ...
 Gross Amount :  <input type="text" name="grossamount" id="grossamount"  value="0"  class="form-control  grossamount" readonly />
	...
	Payable Amount	   <input type="text" autocomplete="off" name="totalamount" id="totalamount"  value="0"  class="form-control numbers totalamount" readonly  />
		  
    </div>	
	
	    <div class="col-4">
      
		Paid:  <input type="text" autocomplete="off"  name="paid" id="paid"  value="0" class="form-control numbers paid"  />
 ...
 Discount :  <input type="text" name="discountatend" id="discountatend"  value="0"  class="form-control  discountatend" readonly  />
    ...
    <div id="dis2"   class="col-4 ">
	 ...
	 Discount on Total(%):  <input type="text" name="percentofdicountontaotal" id="percentofdicountontaotal"  value="0" readonly  class="form-control  percentofdicountontaotal"  />
...
$('#datePicker').val(new Date().toDateInputValue());
    ...
    var table = $('#patient_table').DataTable({
        ...
        ajax: "{{ route('reagentransaction.index') }}",
        columns: [
	   	...
	   	$("#medicinecomapnyname_id").html("");
               	...
               	$("#medicine_name").append(optionformedicine);
					...
					///////////////////////   set dynamic select option values from Database ///////////////////
					                    ...
					                    var len = 0;
                    if (response.reagent != null) {
                        len = response.reagent.length;
                    }

                    if (len>0) {
                        for (var i = 0; i<len; i++) {
                             var id = response.reagent[i].id;
                             var name = response.reagent[i].name;                            
...
$('#sample_form').delegate('#regcustomer ','change',function(){
$("#formhide").show();
...
function totalamount(){
  ...
  $("#percentofdicountontaotal").change(function(){
	...
	var discountatend = $("#discountatend").val();
	
	
	var due = grossamount - discountatend - paid;
...
$('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('reagentransaction.store') }}",
    method:"POST",
